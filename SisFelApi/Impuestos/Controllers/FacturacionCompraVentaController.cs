...
public class FacturacionCompraVentaController : ControllerBase
{
    public string Address = "https://pilotosiatservicios.impuestos.gob.bo/v2/ServicioFacturacionCompraVenta?wsdl";

    
    [HttpPost("verificarComunicacion")]
    public async Task<ServiceCompraVenta.respuestaComunicacion> getVerificacionComunicacion(ServiceCompraVenta.verificarComunicacion request)
    {
        ...
        List<FacturaTelecomunicaciones>facturaTxts = new List<FacturaTelecomunicaciones>();
        //
        await Helper.emisionMasiva(facturaCompraVentas,facturaTxts,0,4000,0,3,1);
    ...
    public async Task<respuestaRecepcion> getRecepcionFactura(solicitudRecepcionFactura request, int codigoFactura, int codigoPuntoVenta,int capa)
    {
        ...
        if(facturaBD != null)
        {
            ...
            {
                xcodigoEmision = 2;
                factura.cafc = Helper.evento.Cafccompraventa;
            }
            ...
            else
            {
                ...
                if (Helper.datosbase.Nrofacturapaquetecv == 1 && Helper.datosbase.Nropaquetecv == 1 && (bool)!Helper.evento.Activo)
                {
                ...
                Helper.datosbase.Nrofacturapaquetecv = Helper.datosbase.Nrofacturapaquetecv + 1;

                // GENERACION DE PAQUETE DE FACTURAS FUERA DE LINEA
                if (Helper.datosbase.Nrofacturapaquetecv == Helper.numeroLimiteDeFacturasPorPaquete + 1)
                {
                    Helper.empaquetarFacturas("Impuestos/XML/FacturacionPaquetes/FacturaCompraVenta", Helper.pathArchivoCompreso);

                    Helper.datosbase.Nrofacturapaquetecv = 1;
                    Helper.datosbase.Nropaquetecv = Helper.datosbase.Nropaquetecv + 1;

                    Helper.eliminarArchivos("Impuestos/XML/FacturacionPaquetes/FacturaCompraVenta");
                }
    ...
    [HttpPost("recepcionAnexos")]
    public async Task<respuestaRecepcion> getRecepcionAnexos(solicitudRecepcionAnexos request)
    {
        Helper.Configurar(Address);
        ServicioFacturacionClient servicio = new ServicioFacturacionClient(Helper.binding, Helper.address);
        servicio.Endpoint.EndpointBehaviors.Add(Helper.behaviour);
        try
        {
            var resp = await servicio.recepcionAnexosAsync(new solicitudRecepcionAnexos()
            {
                anexosList = request.anexosList,
                codigoAmbiente = request.codigoAmbiente,
                codigoDocumentoSector = request.codigoDocumentoSector,
                codigoEmision = request.codigoEmision,
                codigoModalidad = request.codigoModalidad,
            ...
            return resp.RespuestaRecepcionAnexos;
        }
        catch (Exception e)
        {
            Console.WriteLine($"{e.Message}");
    ...
    public async Task<respuestaRecepcion> getRecepcionPaqueteFactura(solicitudRecepcionPaquete request, int codigoPuntoVenta, string pathPaqueteCompreso)
    {
        Helper.Configurar(Address);
        ServicioFacturacionClient servicio = new ServicioFacturacionClient(Helper.binding, Helper.address);
        servicio.Endpoint.EndpointBehaviors.Add(Helper.behaviour);
        ...
        try
        {
            ...
            {
                archivo = xarchivo,
                cafc = Helper.evento.Cafccompraventa,
